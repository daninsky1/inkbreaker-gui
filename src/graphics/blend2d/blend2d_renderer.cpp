...
)
{
    ...
    const auto& blFont = static_cast<const Blend2DFont&>(font).getBLFont();
    _blContext.fillUtf8Text(
        BLPoint{static_cast<double>(position.x), static_cast<double>(position.y)},
        blFont,
        text.data(),
        text.size(),
        BLRgba32(paint.getColor().pack32())
